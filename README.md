...
- 宽度模型：
  ...
  - 首尾空格先删除（中间空格保留）
- 显示单元（固定扫描器，不依赖运行环境）：
  - 基字符后的组合标记（组合音标、变体选择符）与肤色修饰符并入该基字符所在单元
  - 区域指示符（国旗）从左到右两两成对视为一个单元
  - 零宽连接符（ZWJ）串起的完整基字符链（如 👨‍👩‍👧）视为一个单元；
    链尾缺少基字符时，仅尾部连接符独立成单元
  - 无基字符的扩展符、孤立连接符各按一个宽 **2** 的退化单元处理
  - 单元宽度与标点限制均由**首个基字符**判定
- 文本能在一行容纳时：**原样输出**一行。
- 否则只能断成**两行**，断点只取**显示单元边界**（以 Unicode 码点位置展示，
  不切散任何字符、更不切开复合字形），且必须同时满足：
...
- Vitest：为宽度模型、显示单元扫描与平局规则建立**独立判据**（测试内置独立实现的 oracle，与被测代码无共享逻辑）
...
src/
  lib/
    width.ts          # 字符宽度（纯函数）
    units.ts          # 显示单元扫描器（组合标记/国旗/连接符合并，固定实现）
    break.ts          # 校验、单元边界候选枚举、唯一断句择优（含保护区间）
    protect.ts        # 选区 → 码点保护区间换算、标记有效性校验
    width.test.ts     # 宽度独立判据
    units.test.ts     # 显示单元独立扫描判据（组合音标/国旗/肤色/连接链/退化）
